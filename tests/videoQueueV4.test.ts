...
let shared: typeof import("../src/services/videoQueue/shared");
let zealmanExecutor: typeof import("../src/services/videoQueue/executors/zealman");
let dreaminaExecutor: typeof import("../src/services/videoQueue/executors/dreamina");
...
before(async () => {
  ...
  shared = await import("../src/services/videoQueue/shared");
  zealmanExecutor = await import("../src/services/videoQueue/executors/zealman");
  dreaminaExecutor = await import("../src/services/videoQueue/executors/dreamina");
...
test("provider work timeout ignores time spent in the local queue", () => {
  const now = Date.now();
  assert.equal(shared.isProviderWorkTimedOut({}, 6, now), false);
  assert.equal(shared.isProviderWorkTimedOut({ providerSubmittedAt: now - 30 * 60 * 1000 }, 6, now), false);
  assert.equal(shared.isProviderWorkTimedOut({ providerSubmittedAt: now - 7 * 60 * 60 * 1000 }, 6, now), true);
});

test("Zealman pending task is lost only after grace, empty queue, and absent history", () => {
  const now = Date.now();
  const submitted = { providerSubmittedAt: now - zealmanExecutor.ZEALMAN_PENDING_GRACE_MS };
  assert.equal(zealmanExecutor.isZealmanPendingTaskMissing(submitted, { queueIdle: true, historyHasPrompt: false }, now), true);
  assert.equal(zealmanExecutor.isZealmanPendingTaskMissing(submitted, { queueIdle: false, historyHasPrompt: false }, now), false);
  assert.equal(zealmanExecutor.isZealmanPendingTaskMissing(submitted, { queueIdle: true, historyHasPrompt: true }, now), false);
});
...
test("queue config stores maxWorkHours and returns the compatibility alias", () => {
  ...
  const compatible = config.addQueueConfigCompatibility({ queueConfig: { maxWorkHours: 9 } });
...
test("Dreamina poll timing uses initial, processing, and queued intervals independently", () => {
  ...
  assert.equal(dreaminaExecutor.nextDreaminaPollDelayMs(queueConfig, undefined, false), 10_000);
  assert.equal(dreaminaExecutor.nextDreaminaPollDelayMs(queueConfig, 2, true), 20_000);
  assert.equal(dreaminaExecutor.nextDreaminaPollDelayMs(queueConfig, 1, true), 60_000);
});
  ...
  const models = [
    {
      ...
      queueConfig: { pollInitialDelaySec: 60, pollMinIntervalSec: 120, pollMaxIntervalSec: 600, maxWorkHours: 9 },
      durationResolutionMap: [{ duration: [5], resolution: ["720p", "1080p"] }],
    },
    {
      type: "video",
      modelName: "multimodal2video:seedance2.0mini",
      queueConfig: { pollInitialDelaySec: 45, pollMinIntervalSec: 30, pollMaxIntervalSec: 90, maxWorkHours: 8 },
      durationResolutionMap: [{ duration: [5], resolution: ["720p"] }],
    },
  ];
  assert.equal(upgradeDreaminaModelsV5(models), 1);
  assert.deepEqual(models[0].queueConfig, { pollInitialDelaySec: 20, pollMinIntervalSec: 20, pollMaxIntervalSec: 60, maxWorkHours: 9 });
...
test("v4 migration restores only timed-out official tasks and is idempotent", async () => {
  ...
  await db("o_vendorConfig").insert({
    id: "dreamina",
    models: JSON.stringify([{ modelName: "multimodal2video:seedance2.0", type: "video", queueConfig: { maxConcurrent: 1, maxWaitHours: 6 } }]),
  });
  ...
  await db("o_videoGenerationTask").insert([
    {
      id: 1, videoId: 1, taskCenterId: 1, vendorId: "dreamina", model: "dreamina:multimodal2video:seedance2.0",
      requestJson: "{}", phase: "failed", status: "failed", state: "生成失败", submitId: "official-submit-1",
      confirmStartedAt: now - 30 * 60 * 1000,
      errorReason: "任务等待超过 6 小时，已标记失败。submit_id=official-submit-1",
      startTime: now - 7 * 60 * 60 * 1000,
    },
    {
      id: 2, videoId: 2, taskCenterId: 2, vendorId: "dreamina", model: "dreamina:multimodal2video:seedance2.0",
      requestJson: "{}", phase: "failed", status: "failed", state: "生成失败",
      errorReason: "任务排队超过 6 小时，已标记失败。", startTime: now - 7 * 60 * 60 * 1000,
    },
    {
      id: 3, videoId: 3, taskCenterId: 3, vendorId: "dreamina", model: "dreamina:multimodal2video:seedance2.0",
      requestJson: "{}", phase: "capacity_wait", status: "queued", state: "排队中", startTime: now - 8 * 60 * 60 * 1000, nextSubmitTime: now,
    },
  ]);

  const first = await migration.migrateVideoQueueV4(db);
  assert.equal(first.restoredOfficialTasks, 1);
  assert.equal((await db("o_videoGenerationTask").where("id", 1).first()).status, "confirming");
...
test("only a local queued task can be cancelled", async () => {
  await db("o_tasks").where("id", 3).update({
    taskId: "video-task-3", version: 1, status: "queued", phase: "capacity_wait", taskType: "video", projectId: 1,
  });
  const cancelled = await queue.cancelQueuedVideoGenerationTask(3, db, { schedule: false });
  assert.deepEqual(cancelled, { taskId: 3, videoId: 3, status: "cancelled", state: "已取消" });
  assert.equal((await db("o_videoGenerationTask").where("id", 3).first()).status, "cancelled");
  assert.equal((await db("o_tasks").where("id", 3).first()).status, "cancelled");

  await db("o_video").insert({ id: 4, state: "生成中" });
  await db("o_tasks").insert({ id: 4, taskId: "video-task-4", version: 1, status: "processing", phase: "submitting", state: "进行中" });
  await db("o_videoGenerationTask").insert({
    id: 4, videoId: 4, taskCenterId: 4, vendorId: "dreamina", model: "dreamina:multimodal2video:seedance2.0",
    requestJson: "{}", phase: "submitting", status: "submitting", state: "提交中", startTime: Date.now(),
  });
