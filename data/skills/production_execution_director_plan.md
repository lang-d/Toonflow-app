...
- 如果 `get_flowData` 失败或超时，必须立刻停止并向用户报告，不能凭空补齐剧本、资产或导演规划上下文。
...
- **空间可量化**：用景别、构图、人物站位/朝向、动作链等可拍摄的画面要素表达氛围；视觉原则可在「②视觉风格与画面基调」中统一定义，后续镜头不得脱离场景资产和参考图自由发挥
- **情绪靠身体**：通过肢体微表情传达（"指尖发颤、瞳孔收缩"代替"他很紧张"）
- **声音可感知**：画内环境音具体到声源（"蜡芯噼啪声、远处风声"）；BGM 只作为后期制作建议，不写入分镜表音效列和视频提示词
...
规划项：构图风格、镜头运动偏好、色调意图、光线原则、摄影语言、禁用风格词
...
- **构图须说明叙事理由**，参考以下情绪-构图映射（按需选用）：
  ...
  - 对角线构图 → 运动 / 冲突 / 紧张
  - 框中框构图 → 囚禁 / 窥视 / 心理距离
- **空间三层分离**：关键画面须规划前景（引导视线）/ 中景（叙事主体）/ 背景（环境压力、叙事信息或空间关系）的层次关系
- 镜头运动默认以静为主，运镜须说明叙事目的（"缓推=靠近角色内心""缓拉=揭示全貌/抽离"）
- **色调意图与光线原则只在导演层统一定义**：例如"现实冷静、低饱和、自然室内光"或"午后日光、生活化暖色、低戏剧化阴影"。后续分镜表只承接为场景视觉状态或资产需求，图片/视频阶段有参考图时以参考图为事实，不重新发明光影色调。
- **禁用风格词**：列出本片不使用的空泛画质词或风格词，如"极致细节""发丝根根分明""电影感堆叠"等，避免后续提示词污染。
...
- 转折点必须用**具体视觉手段**描述（景别跳切、空镜隐喻、动作骤变、空间反转等），不依赖台词解释；如需视觉气氛变化，必须落到导演层视觉原则或场景资产状态，不逐镜随意改光影色调
...
约束：
- 情绪目标可以有上位意图，但必须同时写清表演出口（身体动作、面部肌肉、视线、呼吸、姿态、手部细节或语气）；例如"偷偷心动：嘴角压不住、视线快速移开、手指无意识摩挲杯沿"，禁止只写"开心/担忧/坚定决绝"等抽象词
- 氛围方向先用情绪和行为目标描述（如"压迫""疏离""温馨"），可补充该场承接的视觉原则或场景资产状态（如"白天候车大厅""雨夜巷口""火光版客厅"），不得逐镜自由发明与资产冲突的光影色调
- **镜头意图写"为什么"**（"用特写让观众看到她眼里的犹豫"），而非"怎么拍"（"用特写拍脸"）
- **场景语义→镜头方案参考**（为每场选择最匹配的方案方向）：
  - 开场/定场 → 大远景 + 缓推至主体
  - 角色登场 → 全景/中景 + 微仰 + 背景轮廓关系
  - 对话交锋 → 中景/近景 + 正反打 + 守视轴
...
规划项：环境音设计、沉默运用、全片 BGM 后期方向

约束：
- 环境音具体到可感知声源（"蝉鸣 / 溪水 / 市井叫卖 / 雨滴檐角"），每场标注 1~2 个核心环境音
- 标注运用沉默手法的关键瞬间（关键情感瞬间只留环境音或全静）
- **画内音效与 BGM 分离**：分镜表 `音效` 列只承载画内环境音 + 动作音；BGM 只写为后期制作建议，说明音乐风格、情绪弧线、节奏语言、乐器倾向和避免项，不进入视频提示词，不传给视频模型
...
- 大段落间可用叠化/淡入淡出做柔性过渡
- 标注全片视觉连续性锚点：角色位置、服装状态、道具状态、场景视觉状态和导演视觉原则在跨场景时保持一致的关键点；落到生成阶段时以场景资产和参考图为准
