...
description: 3D黏土定格场景衍生图视觉手册。将资产事实与视觉设计推导编译为可复用的图片提示词。
metaData: art_skills
---
# 黏土定格动画场景衍生资产生成 · 约束手册
...
本手册负责把已确认事实编译为3D黏土定格场景衍生图提示词，不负责创作剧情、身份、关系、地点、品牌、能力或世界规则。
...
```text
3D黏土定格场景衍生设定图，based on the reference scene design，same location，single image，
...
微缩布景的墙体、地台、道具接触点和机位空间可实际搭建，尺度统一，保持一致的材质响应和可追溯光源，
