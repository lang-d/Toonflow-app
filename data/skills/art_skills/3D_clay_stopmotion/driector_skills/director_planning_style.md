...
黏土风格必须保留实体制作的重量、尺寸和不完美。只转译剧本已有事实；场景、道具和动作若超出实体偶与微缩布景的可执行范围，应标记风险或提出不改变剧情的简化方式。
...
- 先确定角色偶、门窗、家具和手持道具的实体尺度与相对比例，跨场景保持一致。
- 指纹、压痕、纤维和接缝用于证明手工材质，但不能覆盖五官和关键道具信息。
- 软黏土、纸张、木材、金属替代物要有不同形变和接触反馈；不能统一成光滑 CG 表面。
- 景深来自微缩布景的真实层次。前景遮挡应预留动画师操作和镜头观察通道。
...
动作拆成预备、受力、位移、接触和稳定五类关键姿态。角色每次转身、拿取、坐下或碰撞都记录重心和接触点；相邻镜头必须延续手位、道具朝向和形变状态。快速动作允许抽帧和替换件，但结果不能无因出现。

场景轴线由布景中的门、桌边或道路建立。机位优先固定、轻推和可实现的横移；越轴需通过角色实际绕位或正轴重建。禁止以自由飞行镜头暴露布景背面和操作缺口。
...
使用能被微缩场景解释的柔光与局部实景光，控制阴影跳动；灯光变化必须是剧情事件或时间变化。表演依靠姿态、头部角度、手势和停顿，不追求真人级连续微表情。声音强调材质摩擦、轻微碰撞和手工世界的尺度，静默可突出等待；配乐不覆盖动作节拍。

## 导演规划应用检查

每场写明微缩尺度、材质重点、布景锚点、轴线、角色初末姿态、道具接触状态、关键替换件、机位覆盖、灯光连续和制作风险。事实不足时待确认。

## 禁止项

禁止完全光滑和连续运动；禁止无重力漂移、穿模与无限变形；禁止镜头穿过实体障碍；禁止固定剧情及生成或渲染参数。
