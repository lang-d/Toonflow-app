...
description: 90年代日式动画角色衍生图视觉手册。将资产事实与视觉设计推导编译为可复用的图片提示词。
metaData: art_skills
---
...
本手册负责把已确认事实编译为90年代日式动画角色衍生图提示词，不负责创作剧情、身份、关系、地点、品牌、能力或世界规则。
...
```text
90年代日式动画角色衍生设定图，same person as reference image，based on the parent character design，
{必须保持的 assetFoundation 身份、面容、体态、发型基准与连续性锚点}，
{visualDesignRationale 确认的衍生目的、识别度策略与变化边界}，
{本次唯一变化：服装/发型/妆容/配饰/表情或状态}，年代相符的简化服装结构、清楚轮廓与克制表情，发片和衣褶服从有限动画张数，
