...
扁平不等于信息减少，而是用有限视觉变量建立明确层级。只转译剧本已有的人物、行动、场所和道具，不增加情节事实；题材手册决定叙事重点，本手册负责让重点在简化画面中仍然准确可读。
...
为每场建立基础色、人物识别色和事件强调色。强调色只在关键行动或信息发生时移动；关系变化可通过人物色块接近、分离或被环境色吞没表现。阴影使用统一方向和有限级数，不加入写实反射与复杂体积光破坏语言一致性。
...
- 每镜只设一个第一阅读点，靠面积、明度或唯一强调色建立，不能同时用多个高饱和元素争夺注意。
- 人物必须保持可识别剪影。重叠发生时，用轮廓缺口、色差或前后位移分开肢体和关键道具。
- 空间深度依赖遮挡、尺度、垂直位置和少量投影；透视层级最多保持易读的前中后关系。
- 同一地点固定门、桌、窗、通道等锚点的相对位置，背景简化不能导致入口出口漂移。
...
先定义场景轴线和角色移动线，再把动作拆成可辨认的起始、变化和结果形态。对话正反打保持人物左右位置；越轴必须通过正面重置或人物跨越画面中线呈现。镜头运动优先使用平移、推近和构图内运动，旋转、复杂透视穿越只在信息无法通过简单方式表达时使用。

表演使用姿态角度、手臂形状、身体重心和停顿传情，不用细碎面部写实。动作节奏允许图形化压缩，但接触、交接和方向不能省略。
...
声音以画内动作、环境循环和段落静默补足被简化的材质信息；音乐只说明节奏密度和进入退出。导演规划必须写明阅读顺序、色彩角色、空间锚点、初末站位、运动方式、关键剪影、轴线和连续性风险。

## 禁止项

禁止渐变和装饰纹理无限叠加；禁止为活泼感让所有元素持续漂移；禁止用抽象图标替代剧本要求的真实行动结果；禁止固定剧情、摄影设备和生成参数。
