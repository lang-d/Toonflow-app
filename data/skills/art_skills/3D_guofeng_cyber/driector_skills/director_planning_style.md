...
先确定剧本中的时代、地点、技术和传统事实，再决定两套语言如何相互作用。传统结构负责空间秩序与身份，赛博系统负责信息、控制或能量功能；没有剧本依据时，不新增植入体、全息装置、宗教符号或技术规则。
...
- 每场明确“传统为主、科技介入”或“科技占据、传统残留”，不能让两者平均堆满画面。
- 建筑中轴、檐口、廊道和门阙建立地理；数据线路、屏障和光带沿既有结构生长，不随镜头改道。
- 色彩设置传统基底色、系统状态色和唯一警示色。霓虹只说明系统边界或状态变化，不作为通用装饰。
- 纹样和界面必须有可读功能；同场限制符号家族，避免汉字、阵法、电路和粒子同时争抢。
...
记录真实空间轴线、人物视线轴及系统信息出现的位置。角色读取、触发、阻断或穿越系统时，写清输入动作、反馈、环境后果和角色反应。越轴通过地标、角色移动或系统界面方位重建，不能用粒子遮挡掩盖换边。

传统礼仪站位与科技权限若冲突，应明确哪一种权力在当前场景占优，并通过高差、门禁、光区或可接近距离表现。战斗和追逐保持攻击方向、能量来源与落点一致。
...
木、石、丝织与金属保留传统触感；发光表面只属于有功能的装置。人物表演先回应现实行动，再回应界面反馈，避免只盯着空中信息。声音区分实体环境、机械反馈和系统提示；传统器乐与电子质地只描述功能关系，不规定曲目。

## 导演规划应用检查

每场输出融合主次、传统地标、系统功能、色彩状态、空间与视线轴、人物权限站位、触发链、镜头覆盖、特效和资产风险。任何技术规则必须来自剧本或标记待确认。

## 禁止项

禁止固定比例和元素清单；禁止无功能霓虹、漂浮汉字和粒子雨；禁止技术特效改变剧情结果；禁止输出生成模型、渲染器或参数。
