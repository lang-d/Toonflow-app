...
description: 3D动漫渲染道具衍生图视觉手册。将资产事实与视觉设计推导编译为可复用的图片提示词。
metaData: art_skills
---
...
本手册负责把已确认事实编译为3D动漫渲染道具衍生图提示词，不负责创作剧情、身份、关系、地点、品牌、能力或世界规则。
...
```text
3D动漫渲染道具衍生设定图，based on the reference prop design，same object，
...
结构和握持尺寸可建模，硬软材质、连接件与活动部位有清楚分层，完整物体可见，保持同一道具身份，
