...
description: 3D动漫渲染场景衍生图视觉手册。将资产事实与视觉设计推导编译为可复用的图片提示词。
metaData: art_skills
---
...
本手册负责把已确认事实编译为3D动漫渲染场景衍生图提示词，不负责创作剧情、身份、关系、地点、品牌、能力或世界规则。
...
```text
3D动漫渲染场景衍生设定图，based on the reference scene design，same location，single image，
...
前中后景、碰撞空间和可通行路径清楚，卡通材质在不同光位下保持一致，保持一致的材质响应和可追溯光源，
