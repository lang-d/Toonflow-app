...
description: 3D动漫渲染角色衍生图视觉手册。将资产事实与视觉设计推导编译为可复用的图片提示词。
metaData: art_skills
---
...
本手册负责把已确认事实编译为3D动漫渲染角色衍生图提示词，不负责创作剧情、身份、关系、地点、品牌、能力或世界规则。
...
```text
3D动漫渲染角色衍生设定图，same person as reference image，based on the parent character design，
{必须保持的 assetFoundation 身份、面容、体态、发型基准与连续性锚点}，
{visualDesignRationale 确认的衍生目的、识别度策略与变化边界}，
{本次唯一变化：服装/发型/妆容/配饰/表情或状态}，面部拓扑、头发分组、服装层级和关节活动空间明确，避免只靠复杂配件制造识别度，
