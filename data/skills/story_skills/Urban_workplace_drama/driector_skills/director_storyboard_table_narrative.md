...
不新增公司数据、证据和决策，不输出视觉风格或图片生成文本；字段、时长及资产规则服从通用分镜表契约。
...
信息密集处给观众阅读与理解时间，反制后给权力重新分布的关系镜。独处段只保留影响下一步工作的动作，如删除、重写、拨号或停下；不用咖啡、夜景和揉眉心拼贴“职场疲惫”。
...
- 先建立会议桌、屏幕、门和座次，后续反打保持视线与人物左右。
- 文件、手机和屏幕特写只在内容是剧本事实且改变决策时使用，随后连接阅读者反应与行动。
- 递交、收回、签署、关闭或带走物件记录持有权和状态，不能跨镜跳变。
- 人物起身、靠近屏幕、挡门或离席应对应权限与策略变化。
...
议题改变、证据被提交、权限被行使、策略转向、决定形成或人物离开行动时拆镜。不按发言人机械正反打；能同时保留主导者、承受者和旁观反应时使用关系镜。

### 类型专属应用：声音与转场

通知、电话和门声必须有来源并改变场面。转场通过决定后的执行动作连接，不跳过必要的时间、地点或权限变化。
