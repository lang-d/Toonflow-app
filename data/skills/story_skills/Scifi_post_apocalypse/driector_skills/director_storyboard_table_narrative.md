...
不新增技术功能、资源和敌人，不输出视觉风格与图片生成文本；字段及资产规则服从通用分镜表契约。
...
资源状态变化、设备技术状态反馈、危险升级、路线改变、成员权限变化或人物作出代价选择时拆镜。行动过程必须保留关键输入与结果，不能从发现问题直接跳到设备恢复。
...
- 建立镜说明安全区、危险来源、目标位置和撤退方向，近景后返回空间结果。
- 屏幕、仪表或标记只拍剧本已有信息，并同时说明谁读取、是否理解及采取什么行动。
- 携带物、弹药、食物、防护和伤势随镜头连续；交接必须完整呈现。
- 追逐与撤离保持运动方向，越轴前用绕位、地标或正轴镜重建。

### 类型专属应用：群体与选择

资源争议不用轮流对白拆镜，优先保留分配动作、旁观反应和出口控制。选择镜给人物看见风险、执行选择及他人承受后果的连续链，不能只拍一句宣言。
...
探索段给识别环境和设备反馈的时间；危险段压缩但保留行动因果；后果段确认资源与人员状态。警报、机械声和环境声保持来源与距离，音乐不掩盖状态提示。
