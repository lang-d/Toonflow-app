...
填写景别、机位、动作、视线、时长与声音时遵循通用分镜表契约。本 Skill 不输出视觉风格、图片生成文本或新剧情。
...
只拆导演规划已确认的预期、误导、触发、反应和后果，不新增包袱。一个镜头能完整承载铺垫动作时不要按台词逐句切；信息所有者改变、预期被打破或反应对象改变时才形成明确切点。

### 类型专属应用：笑点镜头链

- 铺垫镜要让关键空间、物件和人物确信可读，但不以异常特写提前标答案。
- 触发镜保留动作完成或事实显现的瞬间，不能在结果出现前切断。
- 反应镜优先给真正承受后果或最后理解的人；多人反应按信息传播顺序组织。
- 回收镜要连接先前事实，并留出短暂余韵或补救动作。
- 同一包袱最多设置一个视觉强调中心，避免推拉、音效和多人夸张同时争抢。
...
先用关系镜建立发起者、对象和旁观者位置，正反打保持视线。为笑点故意越轴必须在动作中让观众看见换边，且下一镜重建。喜剧时长由理解所需时间、触发速度和反应余量决定，不套固定秒数。
...
台词镜头围绕意图变化而非说话人轮切。包袱前减少无关切换，包袱后让沉默、目光或补救承担节奏。转场可用动作或语义回收，但不能为押梗篡改时间地点。
