...
不新增招式、敌人或破坏结果，不输出视觉风格和图片生成文本；镜头字段、时长和资产引用服从通用分镜表规则。
...
以目标变化、攻防权转移、接触结果、策略改变和场地状态变化拆镜。不要把每个肢体动作切碎；一镜能清楚呈现预备到落点时优先保持完整。
...
协作镜按创造机会、接力、保护和结果排序，明确每人贡献。高速段缩短时长但保留关键接触，受挫与恢复段给呼吸、判断和重新发起留量。呐喊不单独构成切点，除非改变伙伴行动或对手判断。
...
- 动作镜记录发起者、攻击方向、目标、阻挡或闪避、接触点及双方落点。
- 先用空间镜建立距离与危险，再进入近景；特写后需返回能确认位置和结果的镜头。
- 屏幕方向稳定代表攻防关系，越轴必须通过绕位、翻越、倒地或正轴冲击让换边可见。
- 伤势、武器、服装和环境破坏状态随镜头累积，不能自动恢复。
...
打击声必须匹配材质、力度与距离；环境余响帮助确认空间。高潮后保留胜负、伤势、资源或关系后果镜，不在特效峰值直接转场。
