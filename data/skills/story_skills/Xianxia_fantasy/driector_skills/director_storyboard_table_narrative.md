...
不新增法术、法器、境界和结果，不输出视觉风格或图片生成文本；镜头字段、时长和资产引用服从通用分镜表契约。
...
规则被说明或触发、能力开始、代价显现、攻防权转移、誓约选择形成或结果落地时拆镜。不把每个法术光效单独成镜，镜头必须服务动作因果和人物关系。

### 类型专属应用：法术与战斗连续

- 先建立真实场地、阵营、人物距离和危险区，再显示法阵、法术路径或能量路径。
- 招式镜记录发起手势或条件、传播方向、目标、阻挡、接触和落点。
- 飞行、瞬移或跨空间动作仅按剧本规则呈现，并用地标、目光或结果重建位置。
- 特效遮挡前后保持人物、兵器、伤势和法器状态一致；近景后返回空间结果。

### 类型专属应用：阶序与关系

仪式或议事先建立席位、高差、入口与领受者位置。誓约、授予、拒绝和惩戒镜保留发起、接受或反抗及群体反应。情感反应不能被特效覆盖。
...
能力准备给规则识别时间，交锋段压缩但保留关键接触，代价和选择段重新放开。术法声音保持同一能力的识别规律，音乐不提前宣布胜负或关系结论。
