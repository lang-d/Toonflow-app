...
不新增历史场面或群众结论，不输出视觉风格、资产 Prompt 或模型参数；通用字段、时长和机位命名服从分镜表技法。
...
先让观众理解地点、阵营和权力关系，再进入个人决定。地理改变、命令传递、阵列启动、个人选择或公共后果发生时拆镜；不得用连续大全景替代因果。

### 类型专属应用：大场面连续性

- 建立镜说明关键地标、行动方向和群体分区，之后的近景必须能归属于已建立区域。
- 命令链按发令、传递、接收、执行和结果组织，不能从台词直接跳到无来源行动。
- 仪式和列阵记录队形、朝向、空位和领队位置，切镜后保持一致。
- 敌我或不同政治方位使用稳定屏幕方向；越轴先用高位地理镜、正轴镜或可见移动重建。

### 类型专属应用：个人与群体

个人决定镜必须连接群体后果；群体反应镜必须说明谁先获知和谁仍未知。特写用于印信、书信、兵器等剧本已有关键事实时，要包含持有者和状态变化，不能神秘化普通道具。
...
议事给信息理解和权力反应留量；行动启动后缩短镜头但保留方向；后果段重新拉开以确认代价。声音帮助区分远近命令、队列运动和空间规模，音乐不覆盖关键信息。
