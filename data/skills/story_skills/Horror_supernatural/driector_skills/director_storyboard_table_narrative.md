...
不创造灵异事件和伤亡，不输出视觉风格或图片生成文本；字段与时长服从通用分镜表契约。
...
正常状态被打破、角色察觉、线索得到确认、威胁位置变化或角色作出逃生选择时拆镜。不要按每次声音或每个回头机械切镜；一镜能维持未知空间时，优先让变化在构图内发生。
...
- 负空间必须对应门口、走廊、窗外或视野盲区等已建立位置，不能每镜随机留空。
- 主观镜头注明观察者、视线目标和看不见的范围；客观镜头不得泄露规划要求隐藏的信息。
- 揭示镜先让观众确认变化，再给角色反应；若角色先知道，则反向安排信息顺序。
- 特写只用于剧本已有线索或状态变化，不把普通物件拍成虚假答案。
...
建立出口、障碍、威胁与人物方位。开门、锁门、拾取、跌倒和逃跑记录手位、门状态与运动方向。越轴、黑场和快速切换都不能改变威胁规则或让角色无因脱离危险。
...
等待时长取决于角色确认信息所需时间；惊吓后保留呼吸、判断和行动结果。声源镜与听者反应保持距离逻辑，静默需在表格中说明其信息作用。音乐不能伪造画内声源。
