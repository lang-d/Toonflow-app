...
不新增心理事实、幻觉与反转，不输出视觉风格或图片生成文本；字段、机位和时长服从通用分镜表契约。
...
话语策略改变、信息被确认、空间控制转移、人物停止掩饰或作出决定时拆镜。不要为每句台词切反打，也不要用碎切制造虚假复杂；一个镜头能观察权力变化时保持它。

### 类型专属应用：主客观镜头

主观段标记观察者、触发和退出点；未经确认的画面不得当作客观事实衔接。镜面、遮挡和浅层信息差不能隐藏同一空间必须成立的位置关系。
...
- 先建立人物、出口和关键道具位置，再进入压迫性单人构图。
- 反应镜给真正接收到信息并改变策略的人，沉默镜注明目光对象和身体任务。
- 道具特写只在持有权、状态或信息价值变化时出现，并连接持有者动作。
- 有动机的轴线变化只有在人物绕位、主客观视点切换或权力翻转已有动作触发时使用，随后重建方向。
...
等待时长服从角色是否需要压迫、试探或做决定。权力转移后给新的空间状态和行动后果留量。物件声、呼吸和环境底噪可以放大注意，但配乐不能替角色贴心理标签。
