...
不新增证据、嫌疑或结论，不输出视觉风格和图片生成文本；字段与时长遵循通用分镜表契约。
...
按导演规划标记线索是首次可见、被角色注意、得到解释还是被验证。首次可见可置于正常关系镜中；角色注意时才给予明确视点或细节；验证时必须连接行动与结果。不能把每个道具都拍成悬疑特写。
...
问题改变、线索被发现、解释转向、验证启动或事实确认时拆镜。调查过程保留动作链，避免发现者伸手后直接跳到结论。揭示前给观众识别事实的时间，揭示后给角色目标变化和行动出口。
...
- 主观镜注明观察者、视线目标与可见范围，不能泄露角色不可能看见的信息。
- 偷听、尾随和搜索先建立人物距离、遮挡与出入口，再切局部信息。
- 对话反应镜按知情变化选择，而非轮流拍脸；谎言成立时保留说话者要保护的对象或行动。
- 回看式揭示必须能对应先前确实出现的镜头事实，不补拍从未建立的关键证据。
...
声音线索写明来源、距离和听见者；非画内配乐不得伪装成证据。转场可连接问题与验证地点，但不能隐去不合理的时间和空间移动。
