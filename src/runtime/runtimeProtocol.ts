...
export type RuntimeMessage =
  ...
  | { type: "runtime:heartbeat"; role: RuntimeRole; pid: number; timestamp: number; sentAt: number; sequence: number }
...
export function startRuntimeHeartbeat(role: RuntimeRole, publish: (message: RuntimeMessage) => void) {
  ...
  const send = () => {
    const now = Date.now();
    sequence += 1;
    publish({ type: "runtime:heartbeat", role, pid: process.pid, timestamp: now, sentAt: now, sequence });
  };
  send();
